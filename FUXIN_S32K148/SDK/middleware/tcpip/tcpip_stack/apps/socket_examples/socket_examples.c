...
#define SOCK_TARGET_MAXHTTPPAGESIZE 4096
#endif
...
sockex_nonblocking_connect(void *arg)
{
  ...
  /* write something */
  uint8_t writebuf[IPERF_BUFSZ];
  for(uint32_t i =0;i <IPERF_BUFSZ ; i++)
  {
	  writebuf[i] = i & 0xff;
  }
  while(1)
  {
	  ret = lwip_write(s, writebuf, IPERF_BUFSZ);
	  ret = lwip_write(s, writebuf, IPERF_BUFSZ);
  }
...
#else
  LWIP_UNUSED_ARG(ticks_a);
  LWIP_UNUSED_ARG(ticks_b);
#endif
}
...
#else
  LWIP_ASSERT("opt == opt2", (opt.tv_sec == opt2.tv_sec) && (opt.tv_usec == opt2.tv_usec));
...
#define SNDSTR2 "ET / HTTP/1.0\r\n\r\n"
  ...
  /* might receive multiple packets - keep reading */
  while (ret > 0) {
    ret = lwip_read(s, rxbuf, SOCK_TARGET_MAXHTTPPAGESIZE);
  }
...
#define SNDSTR1 "G"
  ...
  sys_thread_new("sockex_select_waiter1", sockex_select_waiter, &h2, DEFAULT_THREAD_STACKSIZE, DEFAULT_THREAD_PRIO);
  sys_msleep(100);
  sys_thread_new("sockex_select_waiter2", sockex_select_waiter, &h1, DEFAULT_THREAD_STACKSIZE, DEFAULT_THREAD_PRIO);
  sys_msleep(100);
  sys_thread_new("sockex_select_waiter2", sockex_select_waiter, &h4, DEFAULT_THREAD_STACKSIZE, DEFAULT_THREAD_PRIO);
  sys_msleep(100);
  sys_thread_new("sockex_select_waiter2", sockex_select_waiter, &h3, DEFAULT_THREAD_STACKSIZE, DEFAULT_THREAD_PRIO);
...
socket_nonblocking_connect_test(void* arg)
{
 /* Since we'll be initiating connections, make sure we have an address before going any further */
  wait_for_ip();
...
socket_testtwoselects_test(void* arg)
{
  /* Since we'll be initiating connections, make sure we have an address before going any further */
  wait_for_ip();
...
#if SOCKET_EXAMPLES_RUN_PARALLEL
  sys_thread_new("socket_nonblocking_connect", socket_nonblocking_connect_test, &dstaddr, DEFAULT_THREAD_STACKSIZE, DEFAULT_THREAD_PRIO);
  sys_thread_new("socket_testrecv", socket_testrecv_test, &dstaddr, DEFAULT_THREAD_STACKSIZE, DEFAULT_THREAD_PRIO);
  sys_thread_new("socket_testtwoselects", socket_testtwoselects_test, &dstaddr, DEFAULT_THREAD_STACKSIZE, DEFAULT_THREAD_PRIO);
#else
  sys_thread_new("socket_example_test", socket_example_test, &dstaddr, 2048, 8);
#endif
}
