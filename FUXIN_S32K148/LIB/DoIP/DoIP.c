...
void createAndSendNack(uint16 sockNr, uint8 nackCode)
{
	uint8 txBuffer[8+1];
	uint16 bytesSent;
...
void handleVehicleIdentificationReq(uint16 sockNr, uint32 payloadLength, uint8 *rxBuffer, SoAd_Arc_DoIp_VehicleIdentificationRequestType type)
{
	uint8* txBuffer;
	uint16 bytesSent;
...
static boolean isAuthenticated(uint16 activationType) {
	// IMPROVEMENT: Authentication not supported
	return FALSE;
}
...
static boolean isConfirmed(uint16 activationType) {
	// IMPROVEMENT: Confirmation not supported
	return FALSE;
}
...
static void handleRoutingActivationReq(uint16 sockNr, uint32 payloadLength, uint8 *rxBuffer)
{
	 ...
	 */
	if ((7 == payloadLength) || (11 == payloadLength) || (10 == payloadLength/*NOTE: REMOVE WHEN MOVED TO CANOE8.1*/)) {
...
void handleEntityStatusReq(uint16 sockNr, uint32 payloadLength, uint8 *rxBuffer)
{
	if (0 == payloadLength) {
		uint8* txBuffer = NULL;
...
void handlePowerModeCheckReq(uint16 sockNr, uint32 payloadLength, uint8 *rxBuffer)
{
	// @req SWS_DoIP_00091
	if (0 == payloadLength) {
		uint8* txBuffer = NULL;
...
//		nBytes = SoAd_RecvImpl(SocketAdminList[sockNr].ConnectionHandle, rxBuffer, SOAD_RX_BUFFER_SIZE, MSG_PEEK);
		nBytes = SoAd_RecvImpl(SocketAdminList[sockNr].ConnectionHandle, rxBuffer, SOAD_RX_BUFFER_SIZE, 0);
...
void DoIp_MainFunction() {
	...
	for (i = 0; i < DOIP_MAX_TESTER_CONNECTIONS; i++) {
		...
		if (DOIP_ARC_CONNECTION_REGISTERED == connectionStatus[i].socketState) {
			...
			/*
			 * IMPROVEMENT: Handle initial inactivity timeouts
			 */
